...
def get_color_map(track_ids):
    import colorsys
    n = len(track_ids)
    return {track_id: colorsys.hsv_to_rgb(i / n, 0.8, 0.8) for i, track_id in enumerate(track_ids)}


def create_topdown_visualization(ground_truth, predictions, output_filename='tracks_visualization.mp4'):
    ...
    for item in ground_truth:
        gt_frames[item['FrameId']].append(item)
    for item in predictions:
        pred_frames[item['FrameId']].append(item)
    
    # Determine the range of frames
    all_frames = sorted(set(list(gt_frames.keys()) + list(pred_frames.keys())))

    # Get unique track IDs and create a color map
    all_track_ids = set(item['Id'] for item in ground_truth + predictions)
    color_map = get_color_map(all_track_ids)
    
    # Set up the plot
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(20, 10))
    fig.suptitle('Frame: 0', fontsize=16)  # Initialize frame number title
    ...
    def update(frame):
        ...
        fig.suptitle(f'Frame: {frame}', fontsize=16)  # Update frame number
        
        # Plot ground truth
        for item in gt_frames[frame]:
            color = color_map[item['Id']]
            circle = plt.Circle((item['X'], item['Y']), 15, fill=True, color=color, alpha=0.6)
            ax1.add_artist(circle)
            ax1.text(item['X'], item['Y'], str(item['Id']), ha='center', va='center', fontsize=8)
        
        # Plot predictions
        for item in pred_frames[frame]:
            color = color_map[item['Id']]
            circle = plt.Circle((item['X'], item['Y']), 15, fill=True, color=color, alpha=0.6)
            ax2.add_artist(circle)
            ax2.text(item['X'], item['Y'], str(item['Id']), ha='center', va='center', fontsize=8)
    ...
    anim = animation.FuncAnimation(fig, update, frames=all_frames, interval=200)
    
    # Save the animation as a video file
    anim.save(output_filename, writer='ffmpeg', fps=15)
    plt.close(fig)
...
#    cfg.DATASET.ROOT = "/mnt/ml_data/MMPTracking/"
    ...
    topdown_max = cfg.DATASET.TOPDOWN_MAX
    topdown_pred = cfg.MODELS[cfg.TRAINING.MODEL].PRED_TOPDOWN

    dev = 'cuda'
    batch_size = 1  # keep batch size one to get single sample prediction
    val_dataset, train_dataset = _build_dataset(cfg, build_train=train)
    loader = torch.utils.data.DataLoader(
            train_dataset if train else val_dataset,
    ...
    topdown_pred_list = defaultdict(lambda: defaultdict(list))
    topdown_gt_list = defaultdict(lambda: defaultdict(list))
    ...
    for i, data in enumerate(tqdm(loader)):
        ...
        for v, (target, pred, prev_pred, meta) in enumerate(zip(targetf, predf, prev_predf, metadata_f)):
            ...
            if topdown_pred:
                pred_topdown = pred['topdown']
                gt_topdown = target['topdown']

            scene_name = meta["scene_id"]
            scene_id = f'{meta["scene_id"]}_{meta["cam_no"]}'
            frame_id = meta["frame_no"] + 1 # NOTE: trackeval MOT17 evaluation expects 1-indexed frame_ids
            
            topdown_pred_tracks = []
            assert len(pred["tracks"]) == len(pred_boxes) 
            for idx, (trk, box) in enumerate(zip(pred["tracks"], pred_boxes)):
                trk = track_map[v, trk]
                box = box.tolist()
                ...
                if topdown_pred:
                    topdown_xy = pred_topdown[idx]
                    assert len(pred["tracks"]) == len(pred_topdown) == len(pred_boxes) 
                    topdown_pred_tracks.append(torch.cat([trk[None], topdown_xy], dim=0)) # [3,]
            
            if topdown_pred:
                topdown_pred_tracks = torch.stack(topdown_pred_tracks) # [N, 3]
                topdown_pred_list[scene_name][frame_id].append(topdown_pred_tracks) 

            topdown_gt_tracks = []
            assert len(target["tracks"]) == len(gt_boxes)
            for idx, (trk, box) in enumerate(zip(target["tracks"], gt_boxes)):
                box = box.tolist()
                ...
                if topdown_pred:
                    topdown_xy = gt_topdown[idx]
                    assert len(target["tracks"]) == len(gt_boxes) == len(gt_topdown)
                    topdown_gt_tracks.append(torch.cat([trk[None], topdown_xy], dim=0)) # [3,]

            if topdown_pred:
                topdown_gt_tracks = torch.stack(topdown_gt_tracks) # [N, 3]
                topdown_gt_list[scene_name][frame_id].append(topdown_gt_tracks) 
        ...
        for v1 in range(len(gtm)):
            for v2 in range(v1+1,len(gtm)):
                ...
                for d1,pt in enumerate(predf[v1]["tracks"]):
                    d2 = (predf[v2]["tracks"] == pt).nonzero(as_tuple = True)[0]
                    if d2.shape[0] == 1:
                        d2 = d2.item()
                        n_pred_tracks_pwc += 1
                        g1 = dm[v1][d1]
    ...
    # TOPDOWN EVAL
    # =============================================================
    def merge_unique_tensors(tensors):
        merged = torch.cat(tensors, dim=0)
        return torch.unique(merged, dim=0)
    
    def setup_df(topdown_list, topdown_max):
        scene_df = defaultdict(list)
        for scene_name, frame_dict in topdown_list.items():
            for frame_id, trk_lst in frame_dict.items():
                merged_trk = merge_unique_tensors(trk_lst)
                for trk in merged_trk:
                    trk_id, x, y = trk.tolist()
                    scene_df[scene_name].append({
                        'FrameId':frame_id, 
                        'Id':int(trk_id), 
                        'X':int(x * topdown_max), 
                        'Y':int(y * topdown_max), 
            ...
            scene_df[scene_name] = pd.DataFrame(scene_df[scene_name])
            scene_df[scene_name] = scene_df[scene_name].set_index(['FrameId', 'Id'])
    ...
    if topdown_pred:
        ...
        scene_gt_df = setup_df(topdown_gt_list, topdown_max)
        scene_pred_df = setup_df(topdown_pred_list, topdown_max)

        for scene_name in scene_gt_df:
            gt_df = scene_gt_df[scene_name]
            pred_df = scene_pred_df[scene_name]
            ...
            print('================ TOPDOWN_METRICS ===================================================================')
            print(f'{scene_name=}')
            print(strsummary)
            print('====================================================================================================')

            create_topdown_visualization(gt_df.reset_index().to_dict('records'), pred_df.reset_index().to_dict('records'), output_filename=f'{scene_name}.mp4')
