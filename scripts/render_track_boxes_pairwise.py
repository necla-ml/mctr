...
#    cfg.DATASET.LOCATIONS = {"validation": {"64pm":['industry_safety_0']}}
    ...
    fps = 15
    h, w = [360, 640]
    output_video_file = 'realtime_pred_trackboxes.mp4'
    ...
    for i, data in enumerate(loader):
        ...
        outputs_batch, outputs_track, keep = postprocessor(out, costs, keep_prob=0.9)
        ...
        for v, (clip, target) in enumerate(zip(clipf, targetf)):        
            pred_boxes = rescale_bboxes(trackf["track_boxes"][:,v,:], (W,H))
            gt_boxes = rescale_bboxes(target["boxes"], (W,H)) if target["boxes"].numel() else target["boxes"]

            for trk,box in zip(trackf["tracks"], pred_boxes):
            ...
            pred_tracks = [str(track_map[:,t].max().item()) for t in trackf["tracks"]]
            pred_labels = ["{}".format(track_map[v,t].item()) for t in trackf["tracks"]]        
